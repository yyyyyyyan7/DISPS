...
#include <cassert>
#include <chrono>
#include "ProtoIndex.cpp"
#include "TopicIndex.cpp"
...
int main(int argc, char* argv[]) {
    ...
    proto_index.iniDimandThres(Config::DIM, Config::ALPHA, Config::TEST_MODE, Config::CHECK);
    ...
    for (size_t i = 0; i < text_data.size(); ++i) {
        auto embedding = embeddings_data[i];
        auto text_line = text_data[i];
        ...
        if (publish_mode) {
            ...
            if (topic_found < 0) {
                ...
                proto_index.online_add(embedding, topic_index, text_line);
            }
    ...
    proto_index.go_to_nearest_topic(topic_index, 100);
